...
# once-codex.sh - Run a single issue with Codex CLI in a worktree, then merge back.
# Usage: ./once-codex.sh /path/to/repo/.scratch/03-my-issue.md
...
if [ -z "$1" ]; then
  ...
  echo ""
  echo "Run a single issue with Codex CLI in a worktree, then merge back."
